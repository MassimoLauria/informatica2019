...
"""
ex_name      ='lab08colonne'
ex_functions =['somme_per_colonna_mat'] 
...
MOTIVO DEL FALLIMENTO:
    ...
    def test_mat_0_0(self):
        m=[]
        expected=[]
        result=somme_per_colonna_mat(m)
    ...
    def test_matrice_con_righe_lunghezza_diversa(self):
        m = [[3, 5, 6], [3, 6]]
        messaggio = """

MOTIVO DEL FALLIMENTO:
    La funzione somme_per_colonna_mat su input {0} dovrebbe sollevare
    'TypeError' perché accetta solo una matrice con righe della stessa lunghezza come argomento."""
        messaggio = messaggio.format(repr(m))
        with self.assertRaises(ValueError,msg=messaggio):
            somme_per_colonna_mat(m)
