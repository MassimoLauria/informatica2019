...
MOTIVO DEL FALLIMENTO del test '{0}' per la funzione '{1}':""".format(testcase.name, testcase.fname)
    ...
    def print_params(params):
        res = '\n'.join('{}) {}'.format(i,C(x)) for i,x in enumerate(params,start=1))
...
esercizio_file = 'lab11inversione'
esercizio_funzione = 'inversione_stringa'

casi_di_test = [
    Testcase(name="stringa_vuota",  input=('',), result='',
             explanation="l'inversa della stringa vuota è la stringa vuota"),

    Testcase(name="esempio_testo",  input=('sveglia presto',), result='otserp ailgevs')
]
