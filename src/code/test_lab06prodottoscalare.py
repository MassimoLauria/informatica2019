...
"""
ex_name      ='lab06prodottoscalare'
ex_functions =['prodotto_scalare'] 
...
class TestLab06ProdottoScalare(unittest.TestCase):

    def errormsg(self,lista1,lista2,expected,result):
        messaggio = """

MOTIVO DEL FALLIMENTO:
    Il prodotto scalare delle liste {0} e {1} dovrebbe essere {2}, invece il programma 
    restituisce {3}"""
    ...
    def test_liste_singleton(self):
        ...
        lista2=[7]
        expected=35
        result=prodotto_scalare(lista1,lista2)
...
MOTIVO DEL FALLIMENTO:
    La funzione prodotto_scalare su input {0} e {1} dovrebbe sollevare
    'TypeError' perché accetta solo liste come argomento."""
        ...
        with self.assertRaises(TypeError,msg=messaggio):
            prodotto_scalare(lista1, lista2)
...
MOTIVO DEL FALLIMENTO:
    La funzione prodotto_scalare su input {0} e {1} dovrebbe sollevare
    'ValueError' perché accetta solo liste non vuote."""
        messaggio = messaggio.format(repr(lista1), repr(lista2))
        with self.assertRaises(ValueError,msg=messaggio):
            prodotto_scalare(lista1, lista2)
