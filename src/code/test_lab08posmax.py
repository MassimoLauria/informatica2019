...
MOTIVO DEL FALLIMENTO:
    ...
    def test_massimo_in_testa(self):
        m=[[380, 102, 78], [88, 72, 43]]
        expected=0,0
        result=pos_max_mat(m)
        msg=self.errormsg(m,expected,result)
        self.assertEqual(expected,result,msg=msg)

    def test_piu_massimi(self):
        m=[[88, 72, 43], [380, 1020, 78], [88, 72, 43], [380, 102, 1020]]
        expected=1,1
        result=pos_max_mat(m)
        msg=self.errormsg(m,expected,result)
        self.assertEqual(expected,result,msg=msg)

    def test_piu_massimi_in_riga(self):
        m=[[88, 72, 43], [380, 100, 78], [88, 702, 702], [380, 102, 20]]
        expected=2,1
        result=pos_max_mat(m)
        msg=self.errormsg(m,expected,result)
        self.assertEqual(expected,result,msg=msg)

    def test_massimo_in_coda(self):
        m=[[380, 102, 78], [88, 72, 403]]
        expected=1,2
        result=pos_max_mat(m)
        msg=self.errormsg(m,expected,result)
        self.assertEqual(expected,result,msg=msg)
    ...
    def test_matrice_senza_elementi(self):
        m = [[]]
        messaggio = """

MOTIVO DEL FALLIMENTO:
    La funzione pos_max_mat su input {0} dovrebbe sollevare
    'ValueError' perché accetta solo una matrice con almeno un elemento come argomento."""
